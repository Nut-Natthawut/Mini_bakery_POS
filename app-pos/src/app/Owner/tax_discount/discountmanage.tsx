...
export default function DiscountManage() {
    ...
    (async () => {
      try {
        ...
        if (cfgRes?.success && cfgRes.data) {
          setDefaultPct(Number(cfgRes.data.defaultDiscountPct ?? 0));
          setVatPct(Number(cfgRes.data.taxRatePct ?? 7));
        }
  ...
  try {
    ...
    // ถ้ามีสกุลเงินให้เลือกในหน้าตั้งค่า ก็ใส่เพิ่มได้
    // fd.append("currency", "THB");

    const res = await updateSystemConfig(fd as any);
    if (!res || !res.success) {
      throw new Error((res && (res as any).error) || "บันทึกค่าระบบไม่สำเร็จ");
    }
    alert("บันทึกค่าระบบ (VAT/Default Discount) สำเร็จ ✅");
  } catch (e: any) {
    alert(e?.message || "บันทึกค่าระบบไม่สำเร็จ");
  }
};
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded-xl px-4 py-2 bg-[#7A4E1A] text-white shadow hover:brightness-110"
          >
            บันทึกค่าระบบ (VAT/Default)
          </button>
          <button
